...
START_IN_MS="$(date --date='11 min ago' +%s)"000
NOW_IN_MS="$(date +%s)"000
echo -e "`date "+%F %T"`\t INFO ----LOADING MRv2 JOB HISTORY TO DB BETWEEN $START_IN_MS AND $NOW_IN_MS----"

/usr/bin/python2.7 $DIR/$PYTHON_SCRIPT $START_IN_MS $NOW_IN_MS 
